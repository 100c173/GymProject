...
<div class="container-xxl flex-grow-1 container-p-y">
  <div class="row mb-3">
    ...
    <div class="col-md-4 d-flex justify-content-end align-items-center">
      ...
      <!-- Create User -->
      <a class="btn btn-primary me-1" href="{{route('permissions.create')}}" role="button">
        ...
        <table class="table table-striped">
          ...
          <tbody>
            @foreach ($permissions as $permission)
              <tr>
                <td style="width: 45%"><i class="fab fa-angular fa-xl text-danger me-4"></i> <span>{{$permission->id}}</span></td>
                <td>{{$permission->name}}</td>
                <td>
                  <div class="dropdown">
                    <button type="button" class="btn p-0 dropdown-toggle hide-arrow" data-bs-toggle="dropdown"><i class="bx bx-dots-vertical-rounded"></i></button>
                    <div class="dropdown-menu">
                      <a class="dropdown-item" href="{{route('permissions.show',$permission)}}"><i class="bx bx-show me-1"></i>Show</a>
                      <a class="dropdown-item" href="{{route('permissions.edit',$permission)}}"><i class="bx bx-edit-alt me-1"></i>Edit</a>
                        <a class="dropdown-item" href="javascript:{}" onclick="document.getElementById('remove_service_{{$permission->id}}').submit();"><i class="bx bx-trash me-1"></i>
                          <form id="remove_service_{{$permission->id}}" action="{{ route('permissions.destroy', $permission) }}" method="POST" style="display: none;">
                              @csrf 
                              @method('DELETE')
                          </form>
                          Delete
        ...
        <nav aria-label="Page navigation">
          <ul class="pagination justify-content-center">
            ...
            <li class="page-item {{ $permissions->onFirstPage() ? 'disabled' : '' }}">
              <a class="page-link" href="{{ $permissions->appends(['entries_number' => request('entries_number'), 'name' => request('name'), 'role' => request('role')])->previousPageUrl() }}">
            ...
            @for ($i = 1; $i <= $permissions->lastPage(); $i++)
              <li class="page-item {{ $permissions->currentPage() == $i ? 'active' : '' }}">
                <a class="page-link" href="{{ $permissions->appends(['entries_number' => request('entries_number'), 'name' => request('name'), 'role' => request('role')])->url($i) }}">
                  {{ $i }}
                </a>
              </li>
            @endfor
            ...
            <li class="page-item {{ $permissions->hasMorePages() ? '' : 'disabled' }}">
              <a class="page-link" href="{{ $permissions->appends(['entries_number' => request('entries_number'), 'name' => request('name'), 'role' => request('role')])->nextPageUrl() }}">
